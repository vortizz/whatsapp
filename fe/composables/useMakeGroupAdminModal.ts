...
export function useMakeGroupAdminModal() {
    ...
    const targetMember = useState<{ _id: string, name: string } | null>('make-group-admin-target', () => null)
    ...
    async function confirm() {
        ...
        try {
            ...
            const updated = await useMyAuthFetch(`chat/${chatId.value}/group-admin/${targetMember.value._id}`, {
                method: 'PATCH',
                body: { is_admin: true },
            }) as any
            chatStore.$patch(state => { (state.user as any).groupAdmins = updated.groupAdmins })
            closeModal()
        } catch (error: any) {
