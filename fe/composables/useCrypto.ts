...
export function useCrypto() {
  ...
  async function unwrapGroupKey(
    ...
    try {
      ...
      return crypto.subtle.importKey('raw', rawGroupKey, { name: 'AES-GCM' }, false, [
        'encrypt',
        'decrypt',
      ])
    } catch (err) {
  ...
  async function decryptGroupMessage(
    ...
    const tryDecrypt = (key: CryptoKey) =>
      crypto.subtle.decrypt({ name: 'AES-GCM', iv: base64ToBuf(iv) }, key, base64ToBuf(ciphertext))
