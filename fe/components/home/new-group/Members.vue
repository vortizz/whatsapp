                ...
                <template v-for="(group, i) in users" :key="i">
                    ...
                    >
                        ...
                        <div class="flex-1 flex flex-col justify-center pb-3.5">
                            <div class="text-base max-h-6 text-neutral-950 grow text-ellipsis overflow-hidden dark:text-white">
                                {{ u.name }}
                            </div>
                            <div class="text-sm text-black/60 dark:text-white/60 grow max-h-5 text-ellipsis overflow-hidden">
                                {{ u.about }}
                            </div>
                        </div>
                    </div>
...
async function getUsers() {
    try {
        ...
        users.value = splitByLetter(result.map(u => ({
            _id: u._id,
            name: u.name,
            about: u.about,
        })))
...
function splitByLetter(list) {
    ...
    for (const user of list) {
        ...
        const byLetter = list.filter(u => u.name.trim().toUpperCase().charAt(0) === firstLetter && !u.handled)
