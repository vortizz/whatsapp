<template>
  <div class="absolute right-1 top-1 z-10">
    <div ref="btnRef" class="text-xs"></div>
    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
      ...
      >
        <button v-if="currentUserIsAdmin && !isAdmin" @click.stop="makeGroupAdmin" class="text-black/60 w-full text-left px-4 py-2 hover:bg-stone-400/10 flex items-center gap-3 rounded-xl dark:text-zinc-50 text-sm">
          <div>
        ...
        <button v-else-if="currentUserIsAdmin && isAdmin" @click.stop="dismissGroupAdmin" class="text-black/60 w-full text-left px-4 py-2 hover:bg-stone-400/10 flex items-center gap-3 rounded-xl dark:text-zinc-50 text-sm">
        ...
        <button @click.stop="remove" class="text-black/60 w-full text-left px-4 py-2 hover:bg-stone-400/10 flex items-center gap-3 rounded-xl dark:text-zinc-50 text-sm">
...
<script setup>
const props = defineProps(['_id', 'isMenuOpen', 'btnRef', 'isAdmin', 'currentUserIsAdmin'])
const emit = defineEmits(['toggle-menu', 'make-group-admin', 'dismiss-group-admin', 'remove'])

const btnRef = ref(null)
const menuPos = ref({ vertical: 'bottom', horizontal: 'left' })
...
function handleClickOutside(e) {
  if (props.isMenuOpen && btnRef.value && !btnRef.value.closest('[class*="group"]')?.contains(e.target)) {
    emit('toggle-menu')
  }
}
...
function makeGroupAdmin() {
  emit('make-group-admin', props._id)
  emit('toggle-menu')
}

function dismissGroupAdmin() {
  emit('dismiss-group-admin', props._id)
  emit('toggle-menu')
}
