<template>
  <div class="absolute right-1 top-1 z-10">
    ...
    >
      ...
      >
        <button
          v-if="currentUserIsAdmin && !isAdmin"
        ...
        <button
          v-else-if="currentUserIsAdmin && isAdmin"
          class="text-black/60 w-full text-left px-4 py-2 hover:bg-stone-400/10 flex items-center gap-3 rounded-xl dark:text-zinc-50 text-sm"
          @click.stop="dismissGroupAdmin"
        >
          <svg
            viewBox="0 0 24 24"
            height="24"
...
<script setup>
  const props = defineProps(['_id', 'isMenuOpen', 'isAdmin', 'currentUserIsAdmin'])
  ...
  function handleClickOutside(e) {
    if (
      props.isMenuOpen &&
      btnRef.value &&
      !btnRef.value.closest('[class*="group"]')?.contains(e.target)
    ) {
      emit('toggle-menu')
    }
  }
