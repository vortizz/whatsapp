      ...
      <form @submit.prevent="submit">
        ...
        <div v-else class="absolute right-0 inset-y-0 text-neutral-950">
          <svg
            aria-hidden="true"
            role="status"
            class="inline w-5 h-5 me-3 animate-spin"
            viewBox="0 0 100 101"
            fill="none"
...
<script setup>
  ...
  const { _id, about } = storeToRefs(userStore)
  ...
  async function submit() {
    try {
      loading.value = true
      const about = text.value.slice()
      await useMyAuthFetch('user', {
        method: 'PUT',
        body: { _id: _id.value, about },
      })
      userStore.setAbout(about)
      stopEditing()
    } catch (error) {
