    ...
    >
      ...
      >
        ...
        <div class="px-4 pb-3">
          ...
          <div v-if="selected.length" class="flex flex-wrap gap-1.5 mb-3">
            ...
            >
              ...
              <span class="text-base text-neutral-950 dark:text-white ml-2 mr-1.5">{{
                u.name
              }}</span>
              <button
                class="flex items-center p-0.5 text-black/60 dark:text-white/60 hover:bg-stone-400/20 rounded-full"
                @click="deselect(u)"
              >
        ...
        <div class="flex-1 overflow-y-auto px-2.5 pb-4">
          ...
          <div
            v-else-if="!groupedUsers.length"
            class="text-center py-16 text-sm text-black/50 dark:text-white/50"
          >
            No results found
          </div>
          <template v-for="(group, i) in groupedUsers" v-else :key="i">
            ...
            >
              ...
              >
                <Icon
                  v-if="isSelected(u)"
                  name="mdi:check"
                  class="text-white dark:text-neutral-950 text-sm"
                />
              </div>
  ...
  const groupedUsers = computed(() => {
    ...
    for (const user of list) {
      ...
      const group = list.filter(
        (u) => u.name.trim().toUpperCase().charAt(0) === letter && !handled.has(u._id),
      )
