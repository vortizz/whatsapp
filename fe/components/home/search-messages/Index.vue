<template>
    <aside class="flex-1 min-w-80 bg-white dark:bg-neutral-900 flex flex-col border-l border-gray-200 dark:border-white/10">
        ...
        <div class="px-4 py-2">
            <div class="flex items-center gap-2 rounded-full border border-emerald-500 px-3 py-1.5 bg-transparent">
                ...
                />
                <button v-if="query" @click="query = ''" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 flex items-center">
                    <Icon name="material-symbols:close" class="text-base" />
                </button>
            </div>
        </div>

        <main class="flex-1 overflow-y-auto px-2.5 scrollbar scrollbar-w-2 scrollbar-thumb-gray-300 scrollbar-track-transparent">
            <template v-if="!query">
            ...
            <template v-else>
                ...
                >
                    ...
                    <div class="flex items-center gap-1.5 text-sm text-neutral-950 dark:text-white">
                        ...
                        <span v-html="highlight(msg.text)"></span>
                    </div>
                </div>
            </template>
        </main>
