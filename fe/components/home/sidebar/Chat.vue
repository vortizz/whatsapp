    ...
    <div class="flex-1 flex flex-col justify-center pb-3.5">
      <div>
        <div class="flex items-center">
          <div
            class="text-base max-h-6 text-black dark:text-zinc-50 grow text-ellipsis overflow-hidden"
            :class="countUnreadMessages && !lastMessage?.isMine ? 'font-semibold' : ''"
          >
            {{ name }}
          </div>
          <div
            v-if="!isClearing"
            class="text-xs flex-none font-semibold"
            :class="
        ...
        <div class="flex items-end place-items-center">
          <span
            v-if="!isTyping && lastMessage?.isMine"
            class="text-base leading-none mr-0.5"
            :class="lastMessage?.status === StatusMessage.READ ? 'text-sky-400' : 'text-gray-500'"
          >
            <Icon v-if="lastMessage?.status === StatusMessage.SENT" name="mdi:check" />
          ...
          >
            {{
              isGroup && lastMessage.text
                ? (lastMessage.isMine ? 'You' : lastMessage.senderName) + ': ' + lastMessage.text
                : lastMessage.text || 'X'
            }}
          </div>
          <div class="flex-none">
            <div v-if="!isClearing" class="flex flex-row items-center gap-0.5">
              <div
                v-if="countUnreadMessages && !lastMessage?.isMine"
                class="px-1.5 py-1 rounded-full bg-emerald-500 text-white dark:text-neutral-950 text-xs leading-none font-semibold flex items-center justify-center"
              >
