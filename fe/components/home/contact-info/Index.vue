<template>
    <aside class="flex-1 min-w-80 bg-white dark:bg-neutral-900 flex flex-col">
        ...
        <HomeSearchMessages
            v-if="showSearch"
            :chatIdOverride="directChatId"
            :chatNameOverride="member?.name"
            @close="showSearch = false"
            @goToMessage="onGoToMessage"
        />
        <main v-else class="flex flex-col gap-3 overflow-y-auto">
            <HomeContactInfoProfile :member="member" @search="openSearch" @goToChat="goToChat" />
...
watch(() => props.member, () => { showSearch.value = false })
