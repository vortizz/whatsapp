<template>
  <AvatarPlaceholder
    v-if="isGroup && isFirst"
    :size="28"
    :name="from?.name"
    class="self-start cursor-pointer flex-none"
    @click.stop="$emit('view-member', from)"
  />
  <div
    data-message-bubble
    ...
    :class="
      isFirst ? 'rounded-r-md rounded-bl-md' : !isGroup ? 'rounded-md' : 'rounded-md ml-[38px]'
    "
    @contextmenu.prevent.stop="menuRef.open()"
  >
    <span
      v-if="isFirst"
      class="block absolute left-[-8px] top-0 h-[13px] w-2 text-white dark:text-neutral-800"
    >
      <svg
        viewBox="0 0 8 13"
        height="13"
    ...
    <div
      v-if="isGroup && isFirst"
      class="text-xs font-semibold cursor-pointer w-fit hover:underline"
    ...
    <div
      ...
      :class="[replyTo.isMine ? 'border-emerald-500' : 'border-amber-500', isGroup ? 'mt-1.5' : '']"
    ...
    <HomeMainMessageMenu
      ...
      button-class="bg-white/95 text-gray-500 dark:bg-neutral-800/95 dark:text-white/60"
