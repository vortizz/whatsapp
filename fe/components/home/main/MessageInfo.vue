<template>
  <aside class="flex-1 min-w-80 max-w-sm bg-white dark:bg-neutral-900 flex flex-col">
    <header class="flex items-center gap-3 px-4 py-3">
      <button
        class="flex items-center p-2 rounded-full hover:bg-stone-200 dark:hover:bg-white/10 text-gray-600 dark:text-gray-300"
        @click="$emit('close')"
      >
    ...
    <div v-else-if="info" class="flex-1 overflow-y-auto">
          ...
          <div class="flex items-center gap-4 px-5 py-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {{ info.readAt ? formatDatetime(info.readAt) : '--' }}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-4 px-5 py-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {{ info.receivedAt ? formatDatetime(info.receivedAt) : '--' }}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-4 px-5 py-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {{ info.sentAt ? formatDatetime(info.sentAt) : '--' }}
              </p>
            </div>
          </div>
        </div>
      ...
      <template v-else>
        ...
        <div v-if="info.received.length > 0">
          <div
            class="flex items-center gap-3 px-5 pt-5 pb-2"
            :class="{ 'border-t border-gray-100 dark:border-neutral-700': info.read.length > 0 }"
          >
          ...
          <div
            ...
            :class="{
              'border-t border-gray-100 dark:border-neutral-700':
                info.read.length > 0 || info.received.length > 0,
            }"
          >
...
<script setup>
  ...
  function formatDatetime(dateStr) {
    ...
    const time = date
      .toLocaleTimeString(navigator.language, { hour: 'numeric', minute: '2-digit', hour12: true })
      .toLowerCase()
