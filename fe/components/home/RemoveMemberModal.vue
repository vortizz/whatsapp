        ...
        >
            <div class="w-full max-w-lg rounded-2xl bg-white py-5 px-6 shadow-2xl dark:bg-neutral-800">
                <div class="text-sm mb-5 font-semibold text-neutral-950 dark:text-white">
                    Remove {{ targetMember?.name }} from "{{ groupName }}" group?
                </div>
                ...
                <div class="mt-12 flex justify-end gap-2">
                    ...
                    <button
                        class="rounded-full bg-green-600 dark:bg-green-500 px-6 py-2.5 text-sm font-semibold text-white dark:text-neutral-950 transition-colors hover:bg-rose-700 dark:hover:bg-rose-400 disabled:cursor-not-allowed disabled:opacity-35"
                        :disabled="isSubmitting"
                        @click="confirm"
                    >
                        <span v-if="!isSubmitting">Remove</span>
...
<script setup>
const { isOpen, targetMember, groupName, isSubmitting, closeModal, confirm } = useRemoveMemberModal()
</script>
